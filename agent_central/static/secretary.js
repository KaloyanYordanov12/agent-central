...
(function () {
  ...
  function activateTab(name) {
    ...
    if (name === "ask") maybeShowAskEmptyState();
  }

  // Friendly empty state for the Ask tab before any question is asked.
  function maybeShowAskEmptyState() {
    const box = $("ask-history");
    if (!box || box.querySelector(".ask-qa")) return;  // real Q&A already present
    box.innerHTML =
      '<div class="ask-empty" id="ask-empty">' +
      'Ask about what your agents have been doing. For example:<br>' +
      '<span class="ae-eg">"What has Deal Hunter done today?"</span><br>' +
      '<span class="ae-eg">"Were there any errors this week?"</span><br><br>' +
      '<span class="ae-note">This answer is generated by Claude Haiku and needs ' +
      '<code>ANTHROPIC_API_KEY</code> set on the server. The History tab works with no key.</span>' +
      "</div>";
  }
  ...
  function submitQuestion() {
    ...
    if (empty) empty.remove();   // clear the empty-state hint on first question
