...
- [ ] Phase 1: eval framework + Secretary groundedness (objective). Grader unit-
      tested with mocked Secretary answers (right / wrong / hallucinated). Commit.
- [ ] Phase 2: Job Analyst clear-cut categorical suite + runner under SpendGuard.
      Mocked-analyst unit tests. Commit.
- [ ] Phase 3: scorecard backend (GET scorecard + POST run under caps), persist
      latest, provenance per check, endpoint tests (mocked). Commit.
- [ ] Phase 4: Evaluator agent in the office (desk/zone, sprite, idle/running-evals
      state, WS + activity log, honest "no eval yet"). Commit.
- [ ] Phase 5: scorecard UI (dark console tier): pass rate, per-agent, failing
      cases, honest provenance labels, Run-evals button. Screenshot-verify. Commit.
- [ ] Phase 6: infrequent auto-run + skip-if-unchanged, under the caps. Commit.
- [ ] Phase 7: real end-to-end run (the only spend), under caps. Record exact
      calls + dollars here. Screenshot the populated scorecard.
- [ ] Phase 8: regression (full suite green; report new count vs 109) + README
